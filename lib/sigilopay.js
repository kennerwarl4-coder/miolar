...
/**
 * Busca uma transação — GET /gateway/transactions
 *
 * @param {object} params
 * @param {string} [params.id]                ID da transação (o transactionId retornado na criação)
 * @param {string} [params.clientIdentifier]   O "identifier" que enviamos ao criar a transação
 */
async function getTransaction({ id, clientIdentifier }) {
  if (!id && !clientIdentifier) {
    throw new Error("getTransaction: informe id ou clientIdentifier.");
  }
  ...
  const data = await sigilopayRequest("GET", "/gateway/transactions?" + params.toString());
...
module.exports = { sigilopayRequest, createPixCharge, getTransaction };
